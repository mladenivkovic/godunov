...
# using all available solver, specified in the
# solvers=[] list at the beginning of this 
# script.
#
# This script assumes that there are outputs
# in the following directories:
#  ./bin/<solver> for the Godunov solutions
...
#=====================
    ...
    global srcdir
    # assuming here that at least the EXACT solver has
    # solved all possible/interesting ICs
...
#================================
def read_and_plot_godunov(solver):
#================================
    """
    Read in data of godunov solution of a specific solver
    """ 

    global fig

    solvdir = os.path.join('bin', solver)
    godunovdir = os.path.join(os.getcwd(), solvdir)
    godunovdir = os.path.join(godunovdir, filebase)

    if not os.path.isdir(godunovdir):
        print("Didn't find Godunov solution for solver", solver, 'and ic', filebase)
        return
    else:
        print("Plotting Godunov solution for", solver)
    ...
    label = 'Godunov method using '+solver
...
#================================
    ...
    #  plt.show()
    outdir = os.path.join(os.getcwd(), 'png_compare_all')
